  ...
  return (
    <div
      className={
        hero
          ? "w-full"
          : "px-3 pb-[max(0.6rem,env(safe-area-inset-bottom))] sm:px-4 sm:pb-3"
      }
    >
      <div className={`mx-auto w-full ${hero ? "max-w-2xl" : "max-w-xl"}`}>
        ...
        >
          <textarea
            ...
            rows={1}
            disabled={disabled}
            className={`min-h-9 min-w-0 flex-1 resize-none bg-transparent outline-none placeholder:text-muted-foreground/70 disabled:opacity-60 ${
              hero ? "max-h-60 py-1.5 text-base" : "max-h-52 py-2 text-base sm:text-sm"
            }`}
          />
          <button
            type="button"
            ...
            className="mb-px flex size-9 shrink-0 items-center justify-center rounded-xl bg-primary text-primary-foreground transition-[filter,transform] hover:brightness-110 active:translate-y-px disabled:bg-muted disabled:text-muted-foreground disabled:opacity-70"
          >
